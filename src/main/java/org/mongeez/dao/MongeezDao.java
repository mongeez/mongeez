...
public class MongeezDao {
	...
	public MongeezDao(Mongo mongo, String databaseName, MongoAuth auth) {
		...
		if (auth != null) {
			if ((auth.getAuthDb() == null) || auth.getAuthDb().equals(databaseName)) {
				credentials.add(MongoCredential.createCredential(auth.getUsername(), databaseName, auth.getPassword()
						.toCharArray()));
			} else {
				credentials.add(MongoCredential.createCredential(auth.getUsername(), auth.getAuthDb(), auth
	...
	private void loadConfigurationRecord() {
		...
		if (configRecord == null) {
			if (getMongeezCollection().count() > 0L) {
				...
				configRecord = new BasicDBObject().append("type", RecordType.configuration.name()).append(
	...
	public void runScript(String language, String code) {
		if (StringUtils.isEmpty(code)) {
			// do nothing if no code is passed in
			return;
		}

		String l = StringUtils.isEmpty(language) ? "javascript" : language;

		ScriptType scriptType = ScriptType.fromString(l);

		if (scriptType == null) {
			throw new IllegalArgumentException("Unsupported script language: " + language);
		}

		scriptType.scriptExecutor().runScript(db, code);
	}
