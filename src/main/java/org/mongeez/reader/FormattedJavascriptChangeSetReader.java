...
public class FormattedJavascriptChangeSetReader implements ChangeSetReader {
	...
	private static final Pattern CHANGESET_PATTERN = Pattern.compile(
			"//\\s*changeset\\s+([\\w\\-]+):([\\w\\-]+).*",
			Pattern.CASE_INSENSITIVE);
	private static final Pattern ATTRIBUTE_RUN_ALWAYS_PATTERN = Pattern
			.compile(".*runAlways:(\\w+).*", Pattern.CASE_INSENSITIVE);

	private static final Logger logger = LoggerFactory
			.getLogger(FormattedJavascriptChangeSetReader.class);
			...
			while (line != null) {
				...
				if (newChangeSet != null) {
					...
					throw new ParseException(
							file
									+ " has content outside of a changeset.  "
	...
	private void addScriptToChangeSet(ChangeSet changeSet,
			StringBuilder scriptBody) throws ParseException {
		if (changeSet != null) {
			String body = scriptBody.toString();
			if (body.trim().isEmpty()) {
				throw new ParseException("No JavaScript found for changeset "
						+ toString(changeSet), -1);
			}
	...
	private void parseFileHeader(File file, String line) throws IOException,
			ParseException {
	...
	private ChangeSet parseChangeSetStart(String line) {
		...
		if (changeSetMatcher.matches()) {
			...
			changeSet.setRunAlways(parseAttribute(
					ATTRIBUTE_RUN_ALWAYS_PATTERN.matcher(line), false));
		}
		return changeSet;
	}

	private boolean parseAttribute(Matcher attributeMatcher,
