...
public class FormattedJavascriptChangeSetReaderTest {
	...
	public void testGetChangeSetsAlternateEncoding() throws Exception {
		List<ChangeSet> changeSets = parse(Charset.forName("Cp1252"),
				"changeset_Cp1252.js");
		assertEquals(changeSets.size(), 2);
		assertChangeSetEquals(
				changeSets.get(0),
	 ...
	 * valid test. For example, a file that included only ASCII characters would
	 * not be a valid test of the difference between ASCII, UTF-8, and Cp1252
	 * encodings, as ASCII is a subset of the other two encodings.
	 */
	@Test
	...
	private List<ChangeSet> parse(Charset charset, String fileName) {
		FormattedJavascriptChangeSetReader reader = charset != null ? new FormattedJavascriptChangeSetReader(
				charset) : new FormattedJavascriptChangeSetReader();
		File file = new File("src/test/resources/org/mongeez/reader/"
				+ fileName);
	...
	private void assertChangeSetEquals(ChangeSet actual, String expectedAuthor,
			String expectedChangeId, boolean expectedRunAlways,
			String expectedFile, String expectedBody) {
